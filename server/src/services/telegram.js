...
function initUserBot() {
  ...
  userBot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
    ...
    } catch (e) { console.log('userBot /start error:', e.message); }
  });
  ...
  userBot.onText(/\/help/, async (msg) => {
    ...
    } catch (e) { console.log('/help error:', e.message); }
  });
  ...
  userBot.on('message', async (msg) => {
    ...
    } catch (e) { console.log('userBot message error:', e.message); }
  });
...
function initAdminBot() {
  ...
  adminBot.onText(/\/start/, async (msg) => {
    ...
    } catch (e) { console.log('admin /start error:', e.message); }
  });
...
async function sendToAdmins(text, opts = {}) {
  ...
  for (const chatId of adminChatIds) {
    ...
    } catch (e) { console.log(`ADMIN BOT send error (${chatId}):`, e.message); }
  }
}
...
async function publishToChannel(p) {
  ...
  if (!channelId) { console.log('[publishToChannel] CHANNEL_ID не задан в .env'); return null; }

  const bot = userBot || adminBot;
  if (!bot) { console.log('[publishToChannel] Нет активного бота'); return null; }
  ...
  // ── НОВОЕ: строка с ID букета из базы данных ──────────────────────────────
  const idLine         = p.id        ? `🔑 ID: <b>${escHtml(String(p.id))}</b>\n`                     : '';
...
async function notifySellerApproved(p) {
  ...
  } catch (e) { console.log('Не удалось уведомить продавца:', e.message); }
}
...
async function notifyProduct(p) {
  ...
  await sendToAdmins(
    ...
    `💰 ${escHtml(p.price)} TJS · 📍 ${escHtml(p.city)}\n` +
    sizeLine + giftWhenLine + marketPriceLine + codeLine +
...
function setupCallbacks(onApprove, onReject) {
  ...
  adminBot.on('callback_query', async (q) => {
    ...
    } catch (e) { console.log('callback_query error:', e.message); }
  });
}
...
async function notifyBuyerInquirySent(d) {
  ...
  try {
    ...
    await userBot.sendMessage(d.customer_chat_id,
      `✅ <b>Ваша заявка принята!</b>\n\n` +
      `📦 ${escHtml(d.productTitle || '—')}\n` +
      (price ? `💰 ${price}\n` : '') +
      `\nНажмите кнопку ниже — сообщение уже готово, останется только нажать Отправить.`,
      { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: '✈️ Написать администратору', url: tgLink }]] } }
    );
