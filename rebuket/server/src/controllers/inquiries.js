'use strict';
const { q, getClient } = require('../db/supabase');
...
exports.createInquiry = async (req, res) => {
  try {
    ...
    if (product_id) {
      const { data: prod } = await getClient().from('products').select('title').eq('id', product_id).single();
      productTitle = prod?.title;
    }
    notifyInquiry(data, productTitle).catch(() => {});
...
exports.getInquiries = async (req, res) => {
  try {
    ...
    const { data, error, count } = await query.order('created_at', { ascending: false }).range(off, off + lim - 1);
    if (error) throw new Error(error.message);
    res.json({ data: data || [], total: count || 0, page: +page || 1, limit: lim, total_pages: Math.ceil((count || 0) / lim) });
  } catch(e) { res.status(500).json({ error: e.message }); }
};
...
exports.updateInquiry = async (req, res) => {
  try {
    ...
    if (!['new','done'].includes(status)) return res.status(400).json({ error: 'Неверный статус' });
    const { data, error } = await getClient().from('inquiries').update({ status }).eq('id', req.params.id).select().single();
    if (error) throw new Error(error.message);
    res.json(data);
  } catch(e) { res.status(500).json({ error: e.message }); }
};
...
exports.getStats = async (req, res) => {
  try {
    ...
    const [{ data: products }, { data: inquiries }, { data: byDay }] = await Promise.all([
      ...
      sb.from('inquiries').select('created_at').gte('created_at', new Date(Date.now() - 30*86400000).toISOString()),
    ]);
    ...
    const iStats = { total: (inquiries||[]).length, new_inq: (inquiries||[]).filter(i=>i.status==='new').length };
    ...
    const days = Object.entries(dayMap).map(([d,cnt])=>({date:d,count:cnt})).sort((a,b)=>b.date.localeCompare(a.date));
